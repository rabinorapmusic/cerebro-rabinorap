# ============================================================
# STREAMING HOUSE - RABINO RAP - V9 FINAL ONLINE
# ============================================================
import streamlit as st, requests, base64, os

st.set_page_config(page_title="STREAMING HOUSE", page_icon="🎵", layout="wide", initial_sidebar_state="expanded")

st.markdown("""
<style>
.stApp { background: #000; color: white; }
.house-title { text-align: center; font-size: 60px; font-weight: 900; letter-spacing: 6px; }
.house-subtitle { text-align: center; color: #999; margin-bottom: 30px; }
.result { background: #111; border: 1px solid #222; border-radius: 14px; padding: 16px; margin: 8px 0; }
.platform { background: #0f0f0f; border: 1px solid #222; border-radius: 16px; padding: 20px; text-align: center; }
</style>
...
with st.sidebar:
    ...
    if st.session_state.spotify_token:
        st.markdown("🟢 **SPOTIFY ONLINE**")
    else:
        st.markdown("🔴 **SPOTIFY OFFLINE**")
...
with c1: q=st.text_input("Buscar", "Rabino Rap")
with c2: typ=st.selectbox("Tipo", ["track","artist","album"], 0)
    ...
    for key in ["tracks","artists","albums"]:
        if key in res and res[key]["items"]:
            for it in res[key]["items"]:
                ...
                with a1:
                    if img: st.image(img, width=90)
                with a2:
                    st.markdown(f'<div class="result"><h4>{name}</h4><p>{sub}</p></div>', unsafe_allow_html=True)
                with a3:
