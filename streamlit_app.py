...
def razonar(
    ...
    resultado.append(
        f"🔢 Palabras analizadas: "
        f"{len(analisis['palabras'])}"
    )
    ...
    if conocimiento:
        ...
        resultado.append(
            "📚 No encontré conocimiento "
            "almacenado relacionado."
        )

    if analisis["intencion"] == "aprendizaje":

        resultado.append(
            "💡 La orden parece estar "
            "relacionada con aprendizaje."
        )
        ...
        resultado.append(
            "⚙️ La orden fue recibida y "
            "procesada por el núcleo."
        )
...
def estado_cerebro():
    ...
    return {
        ...
        "Conocimiento":
            len(memoria["conocimiento"]),

        "Experiencias":
            len(memoria["experiencias"]),
...
with col2:
    ...
    if st.button(
        ...
        st.session_state.respuesta = (
            ...
            f"📚 Conocimiento: "
            f"{evolucion['conocimiento']}\n"

            f"🧠 Experiencias: "
            f"{evolucion['experiencias']}\n\n"

            "CEREBRO OMEGA registró "
            "el ciclo en su memoria."
        )
...
st.subheader(
    "📡 ALIMENTAR CEREBRO OMEGA"
)

st.caption(
    "CEREBRO OMEGA puede consultar "
    "información externa y prepararla "
    "para su memoria."
)
...
if st.button(
    ...
    if concepto_externo.strip():
        ...
        with st.spinner(
            "🧠 CEREBRO OMEGA está investigando..."
        ):
        ...
        if resultado.get("ok"):
            ...
            st.caption(
                f"🌐 Fuente: "
                f"{conocimiento['fuente']}"
            )

            # Guardamos temporalmente
            # para que el usuario pueda decidir
            st.session_state.conocimiento_externo = (
                conocimiento
            )
...
# ============================================================
# GUARDAR CONOCIMIENTO EXTERNO
# ============================================================

if "conocimiento_externo" in st.session_state:

    externo = st.session_state.conocimiento_externo
    ...
    if st.button(
        ...
        if aprendido:
            ...
            st.success(
                "🧠 CEREBRO OMEGA incorporó "
                "el conocimiento a su memoria."
            )
